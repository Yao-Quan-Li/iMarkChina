...
if ($Mark_Get_Type_Action == 'post') {
    ...
    if (array_key_exists($Mark_Get_Name_Action, $Mark_Posts_Action)) {
        ...
        $Mark_Data_Action = unserialize(file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Data/' . $Mark_Post_Id_Action . '.Mark'));
    } else {
        Mark_404();
    }
    ...
    if (array_key_exists($Mark_Get_Name_Action, $Mark_Pages_Action)) {
        ...
        $Mark_Data_Action = unserialize(file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Data/' . $Mark_Post_Action['file'] . '.Mark'));
    }else {
        Mark_404();
    }
} else {
