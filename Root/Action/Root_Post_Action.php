...
{ 
    ...
    if (isset($_GET['state'])) {
        if ($_GET['state'] == 'draft') {
            $state = 'draft';
            $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/draft.php';
        } else
            if ($_GET['state'] == 'delete') {
                $state = 'delete';
                $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/delete.php';
            } else {
                $state = 'publish';
                $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/publish.php';
            }
    } else {
        $state = 'publish';
        $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/publish.php';
...
function delete_post($id)
{
    ...
    file_put_contents($index_file, "<?php\n\$Mark_Posts_Action=" . var_export($Mark_Posts_Action, true) .
        "\n?>");
    if ($state != 'delete') {
        $index_file2 = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/delete.php';
        include $index_file2;
        $Mark_Posts_Action[$id] = $post;
        file_put_contents($index_file2, "<?php\n\$Mark_Posts_Action=" . var_export($Mark_Posts_Action, true) .
            "\n?>");
    } else {
        unlink($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Data/' . $id . '.Mark');
    }
}
function revert_post($id)
{
    ...
    unset($Mark_Posts_Action[$id]);
    file_put_contents($index_file, "<?php\n\$Mark_Posts_Action=" . var_export($Mark_Posts_Action, true) ."\n?>");
    $index_file2 = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Post/Index/' . $prev_state .'.php';
    include $index_file2;
    $Mark_Posts_Action[$id] = $post;
