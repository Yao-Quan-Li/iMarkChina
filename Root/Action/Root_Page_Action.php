...
if (!is_dir($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Data/')) mkdir($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Data/',0777);
function load_pages() {
    global $state, $index_file, $Mark_Pages_Action;
    if (isset($_GET['state'])) {
        if ($_GET['state'] == 'draft') {
            $state = 'draft';
            $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Index/draft.php';
        } else if ($_GET['state'] == 'delete') {
            $state = 'delete';
            $index_file = $_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Index/delete.php';
        } else {
            $state = 'publish';
...
function delete_page($id) {
    ...
    if ($state != 'delete') {
        ...
        unlink($_SERVER['DOCUMENT_ROOT'] . '/Index/Data/Page/Data/' . $page['file'] . '.Mark');
    }
}
